...
Page({
  ...
  onClickThtask: function(orderno){
    var task = this.data.cpjlist.find(t => t.TC_TTU01 == orderno);
    if(!task){
      return;
    }
    ...
    //   return;
    // }
    var url = '../cpjjy/cpjjy?rwbh=' + orderno +
      '&pm=' + task.TC_TTU13 +
      '&ljbh=' + task.TC_TTU03
    wx.navigateTo({
      url
  ...
  getCpjlist: function () {
    var context = this;
    console.log("request getCpjlist");
  ...
  scanCode: function () {
    ...
    wx.scanCode({
      success: function (res) {
        context.onClickThtask(res.result);
      },
      fail: function (res) {
      }
    })
